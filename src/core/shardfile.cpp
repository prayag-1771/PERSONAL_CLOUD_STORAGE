...
void read_padded(ifstream& in, uint8_t* dst, size_t want) {
    in.read(reinterpret_cast<char*>(dst), static_cast<streamsize>(want));
    const size_t got = static_cast<size_t>(in.gcount());
    if (got < want) fill(dst + got, dst + want, uint8_t{0});
...
bool split_stream(const fs::path& stream_path,
                  const array<fs::path, 4>& shard_paths,
                  uint64_t& shard_size, string& error) {
    error_code ec;
    ...
    ifstream a(stream_path, ios::binary);
    ifstream b(stream_path, ios::binary);
    if (!a || !b) {
        error = "cannot read " + stream_path.string();
        return false;
    }
    b.seekg(static_cast<streamoff>(half), ios::beg);

    ofstream out[4];
    for (int i = 0; i < 4; i++) {
        out[i].open(shard_paths[i], ios::binary | ios::trunc);
        if (!out[i]) {
    ...
    vector<uint8_t> d0(slice), d1(slice), p0(slice), p1(slice);

    uint64_t done = 0;
    while (done < half) {
        const size_t n = static_cast<size_t>(min<uint64_t>(slice, half - done));
...
bool join_shards(const array<fs::path, 4>& shard_paths,
                 uint64_t shard_size, uint64_t stream_size,
                 const fs::path& stream_path, string& error) {
    array<bool, 4> present{};
    for (int i = 0; i < 4; i++) {
        error_code ec;
    ...
    ifstream in_a(shard_paths[static_cast<int>(pair.a)], ios::binary);
    ifstream in_b(shard_paths[static_cast<int>(pair.b)], ios::binary);
    if (!in_a || !in_b) {
    ...
    const size_t slice = config::kIoBufferSize;
    vector<uint8_t> buf_a(slice), buf_b(slice);
    vector<uint8_t> d0, d1;

    uint64_t done = 0;
    while (done < shard_size) {
        const size_t n =
            static_cast<size_t>(min<uint64_t>(slice, shard_size - done));
        ...
        out.seekp(static_cast<streamoff>(done), ios::beg);
        out.write(reinterpret_cast<const char*>(d0.data()),
                  static_cast<streamsize>(n));
        out.seekp(static_cast<streamoff>(shard_size + done), ios::beg);
